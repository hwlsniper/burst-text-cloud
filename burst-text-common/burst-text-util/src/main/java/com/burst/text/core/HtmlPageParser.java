...
public class HtmlPageParser {
    ...
    private void extractAllImageNodes(NodeList nodes) {
        ...
        for (int i = 0; i < filtered.size(); i++) {
            ...
            if (src != null && src.length() > 0) {
                ...
                sb.append("data:image/jpeg;base64,");
                sb.append(imgBase64);
                tag.setAttribute("data-src", path); //sb.toString()
                String imgHtml = tag.toHtml();
                strText = replace(strText, oldImgHtml, imgHtml);
            }
        }
    ...
    public boolean compile() {
        ...
        //1、获得所有js引用的相对路径和绝对路径，用HashMap存储（key-绝对路径，value-要替换的js标签 如（<script src="../common/server.js"></script>））
        ...
        //2、将所有css的引用改成服务器地址
        extractAllCssNodes(nodes, urlMap);
        //引入css代码到html中
        cssInHtml(urlMap);
        //用完后将HashMap清空
        urlMap.clear();

        //3、将所有img的引用改成服务器地址,引入图片的base64编码写到html中
        extractAllImageNodes(nodes);
