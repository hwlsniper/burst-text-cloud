...
var uploadModel = (function () {
    return {
         ...
         */
        open_upload_dialog: function (title, callback, extra_params, multi, filetype, filedir) {
            multi = multi ? 1 : 0;
            filetype = filetype ? filetype : 'image';
            var params = 'multi=' + multi + '&fileType=' + filetype + '&fileDir=' + filedir;
            layer.open({
                type: 2,
                title: title,
                closeBtn: false,
         ...
         * @param filePath 文件路径
         */
        upload_multi_image: function (title, container_selector, item_tpl_wrapper_id, extra_params, filedir) {
            this.open_upload_dialog(title, function (layero, files) {
                ...
                $.each(files, function (i, item) {
                    ...
                    itemtpl = itemtpl.replace(/\{filedir\}/g, filedir);
                    html += itemtpl;
                });
                $('#' + container_selector).append(html);

            }, extra_params, 1, 'image', filedir);
        },

        show_multi_image: function (title, container_selector, item_tpl_wrapper_id, filedir, images) {
            var tpl = $('#' + item_tpl_wrapper_id).html();
            var html = '';
            if (images.length > 0) {
                $.each(images, function (index, img) {
                    var name = img.substr(img.lastIndexOf("/"));
                    var itemtpl = tpl;
                    itemtpl = itemtpl.replace(/\{id\}/g, index);
                    itemtpl = itemtpl.replace(/\{url\}/g, img);
                    itemtpl = itemtpl.replace(/\{preview_url\}/g, img);
                    itemtpl = itemtpl.replace(/\{filepath\}/g, img);
                    itemtpl = itemtpl.replace(/\{name\}/g, name);
                    itemtpl = itemtpl.replace(/\{filedir\}/g, filedir);
                    html += itemtpl;
                });
            }
